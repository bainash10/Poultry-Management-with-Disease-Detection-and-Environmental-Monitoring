...
if (isset($_GET['update'])) {
  ...
  $edit_state = true;
  $query = "SELECT * FROM `chicken_sale` WHERE id = '$update_id' ORDER BY id ASC";
  ...
  $u_no_chickens = $data['no_chickens'];
  $u_date = $data['date'];
}
...
if (isset($_POST['update-sale'])) {
  ...
  $sql = "UPDATE `chicken_sale` SET  customer='$customer', paid='$paid', l_no='$l_no', u_price='$u_price', no_chickens='$no_chickens', t_amount='$t_amount', date='$date' WHERE id=$update_id";
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            <div class="col-xl-6 col-lg-7">
              <div class="card">
                <div class="card-header">Chicken Sales</div>
                <div class="card-body">
                  <div class="form">
                    <form method="post">
                      ...
                      <div class="row">
                        <div class="col-xl-6">
                          <div class="form-group">
                            ...
                            <input type="number" name="no_chickens" class="form-control" required="" value="<?php echo $u_no_chickens; ?>">
                          </div>
                        </div>
                        <div class="col-xl-6">
                          <div class="form-group">
                      ...
                      <?php if ($edit_state == false) {?>
                        <input type="submit" name="chicken-sale" value="Add Record" class="btn btn-secondary">
                      <?php } else {?> 
            ...
            <div class="col-xl-4 col-lg-7">
              <div class="card">
                ...
                <div class="card-body">
                  <?php
                  $query = "SELECT * FROM `chicken_sale`";
                  $fetch = $mysqli->query($query) or die($mysqli->error.__LINE__);
                  $sold = 0;
                  while($row = $fetch->fetch_assoc()){ 
                    $sold += $row['no_chickens'];
                  }?>
                  <p><b>Sold</b>: <?php echo $sold; ?> chickens</p>

                  <?php
                  $query = "SELECT * FROM `chick_purchase`";
                  $fetch = $mysqli->query($query) or die($mysqli->error.__LINE__);
                  $bought = 0;
                  while($row = $fetch->fetch_assoc()){ 
                    $bought += $row['qty'];
                  ...
                  <p><b>Available</b>: <?php echo $available; ?> chickens</p>
                </div>
              </div>
            </div>
          </div>
                ...
                <div class="card-body">
                  <div class="responsive-table">
                    <table id="chicken" class="table table-bordered">
                      ...
                      <tbody>
                        <?php
                        $query = "SELECT * FROM `chicken_sale` ORDER BY id ASC";
                        $fetch = $mysqli->query($query) or die($mysqli->error.__LINE__);
                        while($row = $fetch->fetch_assoc()){ ?>
                          <tr>
                            ...
                            <td><?php echo $row['no_chickens']; ?></td>
                            <td>
                              <a href="sale_chicken.php?update=<?php echo $row['id']; ?>" class="btn btn-success">Update</a>
                              <a href="database/delete.php?delete_chicken_sale=<?php echo $row['id']; ?>" class="btn btn-danger">Delete</a>
                            </td>
                          </tr>
                        <?php } ?>
                      </tbody>
